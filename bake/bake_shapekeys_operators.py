...
class FACEIT_OT_BakeInfo(bpy.types.Operator):
    '''Affiche des informations détaillées sur la configuration de bake FaceIt'''
    bl_idname = "faceit.bake_info"
    bl_label = "Information sur le Bake"
    bl_options = {'INTERNAL'}

    @staticmethod
    def count_bake_elements(scene):
        """Compte les éléments de bake configurés"""
        # Compter les modifiers configurés pour le bake
        bake_modifiers_count = 0
        total_modifiers = 0
        
        for face_obj in scene.faceit_face_objects:
            for modifier in face_obj.modifiers:
                total_modifiers += 1
                if modifier.bake:
                    bake_modifiers_count += 1
        
        # Vérifier l'état des shape keys
        shapes_generated = scene.faceit_shapes_generated
        
        # Compter les actions de bake configurées
        bake_actions_count = 0
        bake_actions = []
        
        if scene.faceit_bake_sk_to_crig_action:
            bake_actions_count += 1
            bake_actions.append(f"SK->CRig: {scene.faceit_bake_sk_to_crig_action.name}")
            
        if scene.faceit_bake_crig_to_sk_action:
            bake_actions_count += 1
            bake_actions.append(f"CRig->SK: {scene.faceit_bake_crig_to_sk_action.name}")
        
        return {
            'bake_modifiers': bake_modifiers_count,
            'total_modifiers': total_modifiers,
            'shapes_generated': shapes_generated,
            'bake_actions': bake_actions_count,
            'bake_actions_list': bake_actions
        }

    def execute(self, context):
        scene = context.scene
        
        # Obtenir les informations de bake
        bake_info = self.count_bake_elements(scene)
        
        # Message détaillé
        message = f"Configuration de Bake FaceIt :\n\n"
        message += f"Modifiers configurés pour bake : {bake_info['bake_modifiers']}/{bake_info['total_modifiers']}\n"
        message += f"Shape keys générées : {'Oui' if bake_info['shapes_generated'] else 'Non'}\n"
        message += f"Actions de bake configurées : {bake_info['bake_actions']}\n"
        
        if bake_info['bake_actions_list']:
            message += "\nActions configurées :\n"
            for action in bake_info['bake_actions_list']:
                message += f"  - {action}\n"
        
        # Objets FaceIt
        from ..core import faceit_utils as futils
        faceit_objects = futils.get_faceit_objects_list()
        message += f"\nObjets FaceIt : {len(faceit_objects)}\n"
        for obj in faceit_objects[:5]:  # Limite à 5 pour éviter un message trop long
            message += f"  - {obj.name}\n"
        if len(faceit_objects) > 5:
            message += f"  ... et {len(faceit_objects) - 5} autres objets\n"
        
        # Afficher dans la console aussi
        print("\n" + "="*50)
        print("FACEIT BAKE INFO")
        print("="*50)
        print(message)
        print("="*50)
        
        summary = f"Modifiers: {bake_info['bake_modifiers']} | Actions: {bake_info['bake_actions']} | SK: {'✓' if bake_info['shapes_generated'] else '✗'}"
