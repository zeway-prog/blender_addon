...
class FACEIT_PT_BakeExpressions(FACEIT_PT_BaseBake, Panel):
    ...
    def draw(self, context):
        ...
        # Compteur de configuration bake - toujours visible
        from ..bake.bake_shapekeys_operators import FACEIT_OT_BakeInfo
        bake_info = FACEIT_OT_BakeInfo.count_bake_elements(scene)
        
        box = col.box()
        row = box.row()
        status_icon = '✓' if bake_info['shapes_generated'] else '○'
        row.label(text=f"🔥 Bake - Modifiers: {bake_info['bake_modifiers']} | Actions: {bake_info['bake_actions']} | SK: {status_icon}", icon='INFO')
        row.operator('faceit.bake_info', text='', icon='QUESTION')
        col.separator()
...
class FACEIT_PT_DuplicateShapeKeys(FACEIT_PT_BaseBake, Panel):
    bl_label = 'Duplicate Shape Keys'
    bl_idname = 'FACEIT_PT_DuplicateShapeKeys'
    faceit_predecessor = 'FACEIT_PT_ShapeKeyUtils'
    weblink = "https://faceit-doc.readthedocs.io/en/latest/bake/"
    ...
    def draw(self, context):
        ...
        if active_obj and active_obj.type == 'MESH':
            ...
            if active_obj.data and active_obj.data.shape_keys:
                ...
                row.label(text=f"📋 Objet: {active_obj.name} | Shape Keys: {shape_keys_count}", icon='MESH_DATA')
                
                if shape_keys_count > 0:
                    col.separator()
                    row = col.row()
                    row.scale_y = 1.2
                    row.operator('faceit.duplicate_shape_keys', text='Dupliquer toutes les Shape Keys', icon='DUPLICATE')
                    
                    col.separator()
                    box = col.box()
                    box.label(text="ℹ️ Cette opération va créer une copie de l'objet pour chaque shape key")
                    box.label(text="avec l'expression appliquée de manière permanente.")
                    box.label(text="Les copies seront organisées en grille décalée de l'objet source.")
                else:
                    col.separator()
                    col.label(text="⚠️ Aucune shape key à dupliquer (seulement 'Basis')", icon='ERROR')
            else:
                row.label(text="⚠️ L'objet sélectionné n'a pas de shape keys", icon='ERROR')
        else:
            col.label(text="⚠️ Veuillez sélectionner un objet mesh avec des shape keys", icon='ERROR')
